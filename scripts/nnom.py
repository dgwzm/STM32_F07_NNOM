...
def generate_model(model, x_test, per_channel_quant=False, name='weights.h', format='hwc', quantize_method='max_min'):
    ...
    with open(name, 'a') as fp:
        ...
        #output_num = (len(model.output) -1) if type(model.output) is list else 0 换了
        output_num = len(model.output)  if type(model.output) is list else 0
        for layer in L:
            if (is_skipable_layer(layer)):
                continue
        ...
        fp.write('\n /* \n')
        for i in layer_q_list:
            fp.write(str(i)+"  "+str(layer_q_list[i])+"\n")
        fp.write('\n */')
    #with open(r'D:\BaiduYunDownload\LITS_data\nnom-master\nnom-master\examples\uci-inception\layer_q_list.txt', 'a+') as fp:
